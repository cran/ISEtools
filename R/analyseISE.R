...
###
	...
	if(program=="jags") {
	  ...
	  # Convert output to R2WinBUGS format
	  ISE.Bayes = list()
	  ISE.Bayes$sims.array = array(NA, c(chains, (iters - burnin)/thin, Num.ISEs*5 + data$M) )
	  for (i in 1:chains) {
	    ISE.Bayes$sims.array[i,,] = my.coda[[i]]
	  }
	  # R2WinBUGS takes output based on parameter entry, rjags alphabetises, so re-order log10x and sigma
	  for (i in 1:chains) {
